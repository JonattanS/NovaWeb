...
const ReporteSaldosPorCentroNitPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Consultar con_sal para obtener saldos iniciales por Centro/NIT
      const filtrosSal = {
        fuente: 'con_sal',
      ...
      const datosProcessados = procesarReporteSaldosPorCentroNit(responseSal || [], responseHis || []);
  ...
  // Función para procesar los datos del reporte de saldos por centro/NIT
  const procesarReporteSaldosPorCentroNit = (datosSal: any[], datosHis: any[]) => {
    console.log("Procesando reporte de saldos por centro/NIT:", { datosSal, datosHis });
    ...
    // Agrupar saldos de con_sal por centro, cuenta y NIT
    const saldosAgrupados = datosSal.reduce((acc, item) => {
      console.log("Procesando item con_sal:", item);
      
      // Verificar sal_tip para saldos de con_scnt (equivalente a CON_SCNT)
      const salTip = item.sal_tip;
      const esSaldoScnt = salTip === 'scnt' || salTip === 'con_scnt';
      
      console.log("Filtros sal_tip:", { salTip, esSaldoScnt, cor_ano: item.cor_ano, añoInicial });
      
      if (!esSaldoScnt || item.cor_ano !== añoInicial) {
        console.log("Item filtrado por sal_tip o año");
        return acc;
      }
      
      const key = `${item.cto_cod || 'SIN_CTO'}-${item.cta_cod || 'SIN_CTA'}-${item.ter_nit || 'SIN_NIT'}`;
      if (!acc[key]) {
        acc[key] = {
    ...
    datosHis.forEach(item => {
      if (item.clc_cod && item.doc_num > 0 && item.mov_val !== undefined && item.cto_cod && item.ter_nit) {
        ...
        const key = `${item.cto_cod}-${item.cta_cod || 'SIN_CTA'}-${item.ter_nit}`;
    ...
    // Ordenar por centro, cuenta y NIT
    resultado.sort((a, b) => {
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900 flex items-center">
              ...
              Reporte de Saldos por Centro / NIT
            </h1>
          </div>
          ...
          {resultado.length > 0 && (
            <div className="flex gap-2">
              <ExcelExporter
                data={resultado}
                filename={`reporte_saldos_centro_nit_CSV_${new Date().toISOString().split("T")[0]}`}
                sheetName="Reporte Saldos por Centro NIT"
                format="csv"
              ...
              <ExcelExporter
                data={resultado}
                filename={`reporte_saldos_centro_nit_${new Date().toISOString().split("T")[0]}`}
                sheetName="Reporte Saldos por Centro NIT"
                format="xlsx"
                onProgressChange={(progress) => setExportProgress(progress)}
                onGeneratingChange={(generating) => setIsExporting(generating)}
...
export default ReporteSaldosPorCentroNitPage