          ...
          {resultado.length > 0 && (
            <div className="flex gap-2">
              <ExcelExporter
                ...
                filename={`Comprobante de DiarioCVS_${new Date().toISOString().split("T")[0]}`}
                sheetName="Comprobante de Diario"
                format="csv"
              ...
              <ExcelExporter
                data={resultadoFiltrado}
                filename={`Comprobante de Diario_${new Date().toISOString().split("T")[0]}`}
                sheetName="Comprobante de Diario"
                format="xlsx"
                onProgressChange={(progress) => setExportProgress(progress)}
                onGeneratingChange={(generating) => setIsExporting(generating)}
