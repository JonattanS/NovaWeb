          ...
          {resultado.length > 0 && (
            <div className="flex gap-2">
              <ExcelExporter
                ...
                filename={`Libros de Inventarios y BalancesCVS_${new Date().toISOString().split("T")[0]}`}
                sheetName="Libros de Inventarios y Balances"
                format="csv"
              ...
              <ExcelExporter
                data={resultadoFiltrado}
                filename={`Libros de Inventarios y Balances_${new Date().toISOString().split("T")[0]}`}
                sheetName="Libros de Inventarios y Balances"
                format="xlsx"
                onProgressChange={(progress) => setExportProgress(progress)}
                onGeneratingChange={(generating) => setIsExporting(generating)}
